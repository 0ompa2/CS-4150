...
namespace NarrowArtGallery
{
    class Program
    {
        ...
        {
            // Grab the first line, where size[0] = N and size[1] = k
            ...
            // Read the next N lines, and store them in the 2D array
            ...
            // Create 3d array that will hold all possible values, so
            // that we don't have to look them up every time when we recurse
        ...
        private static int maxValue(int r, int uncloseableRoom, int k, int N, int[,] gallery, int[, ,] cache)
        {
            ...
            // If k == N - r, then we have to close a door     
            if (k == N - r)
            {
                // If uncloseableRoom is 0 or 1, then we grab that value, 
                // then recurse on the next row, with the same column value
                ...
                // If uncloseableDoor is 2, then we need to find out the MAX of each column
                // recursing down to the next row within the same column
                else
                {
                    if (cache[r, 2, k] == -1)
                    {
            ...
            // If k < N - r, then we don't have to close a door, but maybe we want to
            else if (k <= N - r)
            {
                // If uncloseableRoom is 0 or 1, then we grab that value, 
                // then recurse on the next row, with the same column value 
                // OR we close nothing on this row, and recurse on the next row
                // with both choices, because we don't know how much smaller k is than N - r
                if (uncloseableRoom != 2)
                {
                    if (cache[r, uncloseableRoom, k] == -1)
                    {
                ...
                // If uncloseableRoom is 2, then we need to find the MAX of either value
                // on this row OR closing nothing on this row, and recursing to the next row
                // with both choices, because we don't know how much smaller k is than N - r
                else
                {
                    if (cache[r, 2, k] == -1)
